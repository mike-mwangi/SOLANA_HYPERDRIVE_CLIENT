...
import { getProjectAccountTransactions } from 'store/slices/payment';
...
const TransactionHistory = () => {
    ...
    const [noPayments, setNoPayments] = React.useState(false);
    const { user } = useAuth();

    React.useEffect(() => {
        // eslint-disable-next-line no-underscore-dangle
        dispatch(getProjectAccountTransactions(project?._id));
    }, [dispatch]);
    React.useEffect(() => {
    ...
    const handleSearch = (event) => {
        ...
        if (newString) {
            const newRows = rows.filter((row) => {
                ...
                const properties = ['user_email', 'amount', 'description', 'isDebit', 'date'];
    ...
    return (
        <MainCard title="Transactions History" content={false}>
            <CardContent>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="center">
                                            <Typography
                                                sx={{
                                                    color: row?.isDebit ? 'green' : 'red'
                                                }}
                                            >
                                                {getDebitText(row?.isDebit)}
                                            </Typography>
                                        </TableCell>
